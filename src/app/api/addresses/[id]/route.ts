...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  const item = await db.select().from(addresses).where(eq(addresses.id, params.id));
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const updated = await db.update(addresses).set(updates).where(eq(addresses.id, params.id)).returning();
